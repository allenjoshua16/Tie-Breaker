...
export default function App() {
  ...
  useEffect(() => {
    if (loading) {
      const timeout = setTimeout(() => {
        setLoading(false);
        setError(
          'The simulation timed out. This may happen with high-complexity queries. Please try again or simplify your request.'
        );
      }, 60000);
  ...
  const analytics: AppAnalytics = useMemo(() => {
    ...
    const avgConfidence = Math.round(
      history.reduce((acc, h) => acc + h.confidence.score, 0) / total
    );

    const completed = history.filter(
      h => h.outcome?.status && h.outcome.status !== 'pending'
    );
    ...
    const totalIterations = history.reduce(
      (acc, h) => acc + (h.followUps?.length || 0) + 1,
      0
    );
    ...
    return {
      ...
      accuracyRate,
      regretAverage: 0,
      popularCategories: { General: total },
      avgIterations
    };
  }, [history]);
  ...
  const handleAnalyze = useCallback(
    async (decision: string, preferences: UserPreferences, images?: string[]) => {
      ...
      try {
        ...
        const commonBiases = Array.from(new Set(historicalBiases))
          .slice(0, 3)
          .join(', ');
        ...
        const historyContext = history
          ...
          .map(
            h =>
              `- ${h.decision}: ${h.outcome?.status || 'Pending'} ${
                h.outcome?.notes ? `(Retrospective: ${h.outcome.notes})` : ''
              }`
          )
          .join('\n');
        ...
        setHistory(prev => {
          ...
          return [normalizedData, ...filtered].slice(0, 10);
        });
  ...
  const handleRefine = useCallback(
    async (answer: string) => {
      ...
      try {
        ...
        const followUp = normalizeDecision({
          ...data,
          decision: `Follow-up Insight: ${answer.slice(0, 30)}...`
        });
  ...
  const updateOutcome = useCallback(
    async (
      ...
      if (
        status === currentResult.outcome?.status &&
        notes === currentResult.outcome?.notes &&
        !committedAt
      ) {
        return;
      }
  ...
  return (
    <div className="min-h-screen flex bg-[#F9F9F9]">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header
          className={`border-b border-border-light bg-white/80 backdrop-blur-md sticky top-0 z-[50] px-6 py-4 transition-all ${
            showHistory ? 'left-80' : 'left-0'
          }`}
        >
          <div className="max-w-6xl mx-auto flex items-center justify-between">
            <div className="flex items-center gap-6">
              ...
              >
                ...
                <div>
                  <h1 className="text-xl font-serif italic font-bold text-[#4A4A4A] leading-none group-hover:text-brand-sage transition-colors">
                    Tiebreaker
                  </h1>
                  <p className="text-[8px] uppercase tracking-widest text-[#9B9B9B] font-bold mt-1 group-hover:text-brand-sage/60">
              ...
              {user ? (
                <div className="flex items-center gap-4 bg-gray-50 pr-2 pl-4 py-1.5 rounded-2xl border border-gray-100">
                  <div className="hidden md:block">
                    <p className="text-[10px] font-black uppercase text-[#4A4A4A] tracking-tighter leading-none">
                      {user.displayName || 'Intel Agent'}
                    </p>
          ...
          <AnimatePresence mode="wait">
            {loading ? (
              ...
              >
                ...
                <p className="font-bold text-[10px] uppercase tracking-[0.3em] text-[#4A4A4A] animate-pulse">
                  Running Deep Simulation
                </p>
              </motion.div>
            ) : result ? (
              <motion.div
          ...
          <div className="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-8">
            <div>
              <div className="flex items-center gap-2 opacity-50 mb-2">
                <div className="w-5 h-5 bg-[#4A4A4A] rounded flex items-center justify-center text-white font-serif text-[10px] font-bold">
                  T
                </div>
            ...
            <div className="flex gap-12 font-bold uppercase tracking-[0.2em] text-[10px] text-gray-400">
              <div className="flex flex-col gap-2">
                ...
                <a href="#" className="hover:text-brand-sage">
                  Security
                </a>
                <a href="#" className="hover:text-brand-sage">
                  Ethics
              ...
              >
                <div className="relative p-12">
                  ...
                  <div className="flex items-center gap-4 mb-10">
                    ...
                    <div>
                      <h1 className="text-3xl font-serif italic font-bold text-[#4A4A4A]">
                        Tiebreaker
                      </h1>
                      <p className="text-[10px] uppercase tracking-widest text-brand-sage font-black">
                  ...
                  <div className="space-y-8">
                    <section>
                      ...
                      <p className="text-lg text-gray-500 font-serif italic leading-relaxed">
                        Tiebreaker is built to dismantle the gridlock of high-stakes
                        choices. We provide a neutral, deep-intelligence layer between
                        your instincts and your actions.
                      </p>
                    </section>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                      <section>
                        ...
                        <ul className="space-y-2 text-[11px] font-bold text-gray-400 uppercase tracking-tight">
                          <li>Brutal Honesty Modeling</li>
                          <li>Risk-Mitigation Simulation</li>
                      ...
                      <section>
                        ...
                        <p className="text-sm text-gray-400 leading-relaxed italic">
                          Move beyond binary pro-vs-con lists. The engine identifies
                          smarter alternatives that balance stability with growth.
                        </p>
                      </section>
                    </div>
